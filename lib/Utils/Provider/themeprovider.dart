...
class ThemeProviderClass extends ChangeNotifier {
  ...
  static _getCurrentTheme() {
    ...
    DateTime startTime = DateTime(now.year, now.month, now.day, 2, 20); // Adjust the start time
    DateTime endTime = DateTime(now.year, now.month, now.day, 20, 0);  // Adjust the end time
  ...
  static _getCurrentIconTheme() {
    ...
    return isDayTime ? _lightIconTheme : _darkIconTheme;
  }
}
